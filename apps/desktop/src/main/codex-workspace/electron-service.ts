...
export function createDesktopCodexWorkspace(owner: WebContents) {
	...
	const rendererUrl = process.env.VETTA_DESKTOP_DEV_URL ?? pathToFileURL(join(rendererRoot, "renderer/index.html")).href;
	...
	const ensureSender = (event: IpcMainInvokeEvent) => {
		if (stopped || owner.isDestroyed() || !isCodexWorkspaceSender({
			owner: event.sender === owner,
			mainFrame: event.senderFrame === owner.mainFrame, url: event.senderFrame?.url ?? ""
		}, rendererUrl)) {
	...
	const create = () => new CodexWorkspaceController({
		...
		confirmProfile: async profile => {
			...
			const result = await dialog.showMessageBox(getWindow(), {
				type: "warning", title: mainT("codex:confirmTitle"),
				message: mainT("codex:confirmMessage"), detail,
				buttons: [mainT("codex:cancel"), mainT("codex:save")], defaultId: 0, cancelId: 0, noLink: true
			});
	...
	const get = async () => {
		...
		if (!current) {
			...
			unsubscribe = current.subscribe(notice => {
				if (!owner.isDestroyed()) { try { owner.send(CODEX_WORKSPACE_CHANNELS.CHANGED, notice); } catch { /* View is gone. */ } }
			});
		}
		return current;
	};
	...
	const dispose = () => { stopped = true; reset(); return retired; };
	const unregisterQuit = registerQuitCleanupParticipant(dispose);
	return {
		attach: async (event: IpcMainInvokeEvent) => { ensureSender(event); const service = await get(); ensureSender(event); return service.attach(); },
		...
		teardown: () => {
			owner.removeListener("render-process-gone", reset); owner.removeListener("did-start-navigation", navigation);
			owner.removeListener("destroyed", reset);
			void dispose().then(unregisterQuit, () => { /* Keep the quit participant when cleanup failed. */ });
		},
	};
}
