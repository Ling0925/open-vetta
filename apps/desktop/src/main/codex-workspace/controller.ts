...
export interface CodexWorkspaceControllerOptions {
	...
	createBackend(profile: CodexWorkspaceProfile, approval: (request: WorkspaceApprovalRequest) => Promise<"accept" | "decline" | "cancel">): WorkspaceBackend;
}
...
export class CodexWorkspaceController {
	...
	constructor(private readonly options: CodexWorkspaceControllerOptions) {
		this.ready = options.readProfile().then(value => {
			if (this.disposed) return;
			this.profile = value; this.phase = value ? "ready" : "setup"; this.changed();
		}, () => { if (!this.disposed) { this.error = "CONFIGURATION"; this.changed(); } });
	}
	...
	async attach(): Promise<{ token: string; snapshot: CodexWorkspaceSnapshot }> {
		...
		await this.ready; this.assertAlive();
		...
		if (this.profile && !this.configuring && !this.sessionClosing) {
			try {
				const backend = this.ensureBackend(); const sessions = await backend.list();
				if (this.backend === backend && !this.configuring) this.sessions = sessions;
			}
			catch (error) { if (version === this.attachVersion) this.error = errorCode(error); }
		}
		this.assertToken(token); this.changed();
		return { token, snapshot: this.snapshot() };
	}
	async execute(token: unknown, value: unknown): Promise<CodexWorkspaceReply> {
		try {
			await this.ready; this.assertToken(token);
			const action = command(value);
			if (action.type === "detach") {
				this.token = undefined; this.approvals.cancelAll();
			...
			if (action.type === "choose") {
				...
				this.assertToken(token); return { ok: true, ...(chosenPath ? { chosenPath } : {}) };
			}
			if (action.type === "configure") {
				await this.configure(token, action.profile); return { ok: true, snapshot: this.snapshot() };
			}
			if (action.type === "open") {
				await this.open(token, action.sessionId); return { ok: true, snapshot: this.snapshot() };
			}
			if (action.type === "send") {
				this.send(action); return { ok: true, acceptedInputId: action.inputId, snapshot: this.snapshot() };
			}
			if (action.type === "stop") {
				...
				this.phase = "stopping"; this.approvals.cancelAll(); this.changed();
				await active.handle.stop(); await active.task;
			...
			await this.closeSession(); return { ok: true, snapshot: this.snapshot() };
		} catch (error) {
	...
	dispose(): Promise<void> {
		...
		this.disposed = true; this.token = undefined; this.generation += 1; this.approvals.cancelAll();
		this.phase = "closed"; this.changed();
		this.closing = Promise.resolve().then(async () => {
			...
			this.active?.unsubscribe(); this.active = undefined;
			await this.backend?.close(); this.backend = undefined; this.listeners.clear();
		});
		return this.closing;
	}
	private snapshot(): CodexWorkspaceSnapshot {
		...
		return {
			instanceId: this.instanceId, revision: this.revision, phase: this.phase,
			...(this.profile ? { profile: { ...this.profile } } : {}),
			...(this.active ? { sessionId: this.active.handle.id } : {}),
			...(this.active?.inputId ? { activeInputId: this.active.inputId } : {}),
			rows: structuredClone(history?.rows ?? []), hasEarlierRows: history?.hasEarlierRows ?? false,
			approvals: this.approvals.list(), sessions: structuredClone(this.sessions),
			...(this.error ? { errorCode: this.error } : {}), ...(this.outcome ? { outcome: this.outcome } : {})
		};
	}
	private async configure(token: unknown, profile: CodexWorkspaceProfile): Promise<void> {
		...
		try {
			...
			this.profile = profile; this.sessions = []; this.error = undefined;
			if (!this.disposed) { this.phase = "ready"; this.changed(); }
			this.assertToken(token);
			this.sessions = await this.ensureBackend().list(); this.assertToken(token); this.changed();
	...
	private async open(token: unknown, sessionId?: string): Promise<void> {
		...
		this.phase = "opening"; this.error = undefined; this.outcome = undefined; this.changed();
		const work = Promise.resolve().then(async () => {
			...
			try {
				...
				if (this.disposed || generation !== this.generation || token !== this.token) {
					await handle.close(); handle = undefined; throw new CodexWorkspaceError("CANCELLED");
				}
				...
				if (this.disposed || generation !== this.generation || token !== this.token) throw new CodexWorkspaceError("CANCELLED");
				if (!this.disposed && generation === this.generation) { this.phase = "ready"; this.changed(); }
			} catch (error) {
				if (handle && (this.disposed || generation !== this.generation || token !== this.token || !this.active)) {
					await handle.close();
					if (this.active?.handle === handle) { this.active.unsubscribe(); this.active = undefined; }
				}
				if (!this.disposed && generation === this.generation) {
					this.error = errorCode(error); this.phase = this.active ? "recovery" : "ready"; this.changed();
				}
				throw error;
			}
		});
		this.exclusive = work;
		try { await work; } finally { if (this.exclusive === work) this.exclusive = undefined; }
	}
	private send(action: Extract<CodexWorkspaceCommand, { type: "send" }>): void {
		...
		if (prior) {
			if (prior.sessionId !== action.sessionId || prior.digest !== digest) throw new CodexWorkspaceError("INPUT_CONFLICT");
			return;
		}
		if (this.phase !== "ready" || active.task || active.handle.snapshot().recovery) throw new CodexWorkspaceError("BUSY");
		active.inputId = action.inputId; this.phase = "running"; this.error = undefined; this.outcome = undefined;
		...
		active.task = Promise.resolve().then(async () => {
			if (this.disposed || active.closing || (this.phase === "stopping" || this.phase === "closing")) return { status: "cancelled" as const };
			return active.handle.prompt(action.text);
		}).then(result => {
			if (this.active !== active || active.closing || this.disposed) return;
			this.outcome = result.status; this.error = result.errorCode;
			this.phase = active.handle.snapshot().recovery ? "recovery" : "ready";
		}, error => {
			if (this.active !== active || active.closing || this.disposed) return;
			this.error = errorCode(error); this.outcome = "failed"; this.phase = "recovery";
		}).finally(() => {
			active.inputId = undefined; active.task = undefined;
			if (this.active === active) { this.approvals.cancelAll(); this.changed(); }
		});
		this.changed();
	}
	private closeSession(): Promise<void> {
		if (this.sessionClosing) return this.sessionClosing;
		const closing = this.doCloseSession(); this.sessionClosing = closing;
		void closing.then(() => { if (this.sessionClosing === closing) this.sessionClosing = undefined; },
	...
	private async doCloseSession(): Promise<void> {
		...
		this.generation += 1; this.phase = "closing";
		if (this.active) this.active.closing = true;
		this.approvals.cancelAll(); this.changed();
		try {
			await this.exclusive?.catch(() => undefined);
			const active = this.active;
			if (active) {
				active.closing = true; await active.handle.close(); await active.task;
				active.unsubscribe(); if (this.active === active) this.active = undefined;
			}
			this.inputs.clear();
			if (this.backend) this.sessions = await this.backend.list();
			if (!this.disposed) { this.phase = this.profile ? "ready" : "setup"; this.error = undefined; this.changed(); }
		} catch (error) {
			if (!this.disposed) { this.phase = "recovery"; this.error = errorCode(error); this.changed(); }
			throw error;
		}
	}
	...
	private requireSession(id: string): ActiveSession {
		this.assertAlive(); identifier(id);
		if (!this.active || this.active.closing || this.active.handle.id !== id) throw new CodexWorkspaceError("SESSION_MISMATCH");
		return this.active;
	}
	private assertAlive(): void { if (this.disposed) throw new CodexWorkspaceError("CLOSED"); }
	private assertToken(token: unknown): asserts token is string {
		this.assertAlive(); if (typeof token !== "string" || !this.token || token !== this.token) throw new CodexWorkspaceError("VIEW_EXPIRED");
	}
	private changed(): void {
		const notice = { instanceId: this.instanceId, revision: ++this.revision };
		for (const listener of [...this.listeners]) { try { listener(notice); } catch { /* Observer isolation. */ } }
	}
}
