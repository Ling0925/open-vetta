...
export function WebAccessSettings(): JSX.Element {
	...
	const enabled = model.state.status === "enabled" || model.state.status === "starting";
		...
		<div className="mx-auto w-full max-w-[680px] px-8 pt-2 pb-8">
			...
			<section className="mb-6 rounded-xl border border-border/50 bg-card/40 p-4">
				<div className="mb-4 flex items-start justify-between gap-4">
					...
					<Switch
						checked={enabled}
						disabled={model.busy || !model.origin.trim()}
						onCheckedChange={(checked) => void (checked ? model.enable() : model.disable())}
						aria-label={t("webAccess.enable")}
					/>
				</div>
				...
				<div className="mt-4 flex flex-wrap items-center gap-2">
					<Button variant="outline" disabled={model.busy || enabled} onClick={() => void model.configure()}>
						{t("webAccess.save")}
					</Button>
					{enabled ? (
						<Button variant="destructive" disabled={model.busy} onClick={() => void model.disable()}>
							{t("webAccess.disable")}
						</Button>
					) : null}
					<span className="text-[12px] text-muted-foreground">{t(`webAccess.status.${model.state.status}`)}</span>
				</div>
			</section>
			...
			<section className="mb-6 rounded-xl border border-border/50 bg-card/40 p-4">
				<div className="flex items-start justify-between gap-4">
					...
					<Button disabled={!enabled || model.busy} onClick={() => void model.pair()}>
						{t("webAccess.pair")}
					</Button>
				</div>
				{model.pairing ? (
