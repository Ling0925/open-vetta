...
export function CodexWorkspacePage(): ReactElement {
	...
	const { state, run } = useCodexWorkspace(attempt);
	const modelChoices = useCodexModelChoices(run, state.connection === "ready" && !state.snapshot?.sessionId);
	const data = state.snapshot;
	const [drafts, setDrafts] = useAtom(codexWorkspaceDraftsAtom);
	const scope = data?.sessionId ?? "new";
	...
	useEffect(() => {
		...
		if (data.outcome === "completed")
			setDrafts((previous) =>
				previous[sent.sessionId] === sent.text ? { ...previous, [sent.sessionId]: "" } : previous,
			);
	...
	const open = async (sessionId?: string) => {
		...
		if (!sessionId && reply.ok && reply.snapshot?.sessionId) {
			...
			setDrafts((previous) =>
				previous.new && !previous[id] ? { ...previous, [id]: previous.new, new: "" } : previous,
			);
		}
	};
	...
	return (
		<section className="flex min-h-0 flex-1 flex-col overflow-auto px-8 pb-8 text-[13px]">
			...
			<header className="flex flex-wrap items-center justify-between gap-3">
				...
				<Button variant="outline" onClick={() => void navigate({ to: "/new-session", search: {} })}>
					{t("native")}
				</Button>
			</header>
			<p className="my-3 text-muted-foreground">{t("limits")}</p>
			{state.connection === "loading" && <output className="block">{t("loading")}</output>}
			{state.connection === "failed" && (
				<Button variant="outline" onClick={() => retry((value) => value + 1)}>
					{t("reconnect")}
				</Button>
			)}
			{code && (
				<div role="alert" className="my-3 text-destructive">
					<p>{t("errorHelp")}</p>
					<details>
			...
			{data && (
				<>
					<output className="block my-3">
						{t(`phases.${data.phase}`)}
						{data.outcome ? ` · ${t(`outcomes.${data.outcome}`)}` : ""}
					</output>
					{!data.sessionId && data.phase !== "opening" && data.phase !== "closing" && (
						<CodexProfileForm
							key={JSON.stringify(data.profile)}
							initial={data.profile}
							runtimeDefaults={data.runtimeDefaults}
							models={modelChoices.models}
							modelsStatus={modelChoices.status}
							reloadModels={modelChoices.reload}
							disabled={!editable || state.pending.includes("configure")}
							save={configure}
							choose={async (field) => {
								const reply = await run({ type: "choose", field });
								return reply.ok ? reply.chosenPath : undefined;
							}}
						/>
					)}
					{!data.sessionId && (
						<div className="my-4 flex flex-wrap gap-3">
							<Button
								disabled={
									!editable || !data.profile || state.pending.includes("configure") || data.phase !== "ready"
								}
								onClick={() => void open()}
							>
								{t("create")}
							...
							{data.sessions.length > 0 && (
								<>
									<label className="self-center" htmlFor="codex-saved-session">
										{t("savedSessions")}
									</label>
									<select
										id="codex-saved-session"
										className="min-w-0 max-w-full rounded-lg border border-border bg-background p-2"
										value={data.sessions.some((item) => item.id === savedSession) ? savedSession : ""}
										onChange={(event) => selectSavedSession(event.target.value)}
									>
									...
									<Button
										...
										disabled={
											!editable ||
											data.phase !== "ready" ||
											!data.sessions.some((item) => item.id === savedSession)
										}
										onClick={() => void open(savedSession)}
									>
										{t("resumeSelected")}
									</Button>
								</>
					...
					{(data.sessionId || data.phase === "opening" || data.phase === "closing") && (
						<div className="my-3 flex flex-wrap gap-3">
							...
							<Button
								variant="outline"
								disabled={state.pending.includes("close")}
								onClick={() => void run({ type: "close" })}
							>
								{t("close")}
							</Button>
						</div>
					...
					{data.approvals.map((approval) => (
						<section
							key={approval.id}
							className="my-3 rounded-xl border border-border p-4"
						...
						>
							...
							<div className="flex gap-3">
								<Button
									variant="outline"
									disabled={state.pending.includes("approval")}
									onClick={() => void run({ type: "approval", approvalId: approval.id, decision: "decline" })}
								>
									{t("decline")}
								</Button>
								<Button
									disabled={state.pending.includes("approval") || !running || data.phase === "stopping"}
									onClick={() => void run({ type: "approval", approvalId: approval.id, decision: "accept" })}
								>
									{t("acceptOnce")}
								</Button>
							</div>
					...
					<CodexTranscript
						rows={data.rows}
						hasEarlierRows={data.hasEarlierRows}
						labels={{
					...
					>
						<label className="block" htmlFor="codex-prompt">
							{t("prompt")}
						</label>
						<Textarea
							id="codex-prompt"
							className="my-2 min-h-24 w-full rounded-lg border border-border bg-background p-3"
							value={draft}
							maxLength={100000}
							onChange={(event) => changedDraft(event.target.value)}
						/>
						...
						<div className="mt-3 flex gap-3">
							...
							<Button
								type="button"
								variant="outline"
								disabled={!running || !data.activeInputId || state.pending.includes("stop")}
								onClick={() =>
									data.activeInputId &&
									data.sessionId &&
									void run({ type: "stop", sessionId: data.sessionId, inputId: data.activeInputId })
								}
							>
								{t("stop")}
							</Button>
