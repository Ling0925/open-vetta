...
import type { CodexModelChoice, CodexWorkspaceProfile, CodexWorkspaceReply } from "../../../shared/codex-workspace";

export function CodexProfileForm({ initial, disabled, save, choose, models = [], modelsStatus = "loading", reloadModels }: {
	...
	const submit = () => {
		...
		void save(source === "vetta" ? { ...runtime, vettaModelKey: modelKey } : { ...runtime, ...(model ? { model } : {}) });
	};
	return <form onSubmit={event => { event.preventDefault(); submit(); }} className="my-3">
		<fieldset disabled={disabled} className="space-y-3">
			<legend className="mb-3 font-semibold">{t("configuration")}</legend>
			<label htmlFor="codex-model-source" className="block">{t("modelSource")}</label>
			<Select value={source} disabled={disabled} onValueChange={next => setSource(next === "vetta" ? "vetta" : "codex")}>
				<SelectTrigger id="codex-model-source"><SelectValue /></SelectTrigger>
				<SelectContent>
			...
			{source === "vetta" && <>
				<label htmlFor="codex-existing-model" className="block">{t("chooseModel")}</label>
				<Select value={modelKey} disabled={disabled || modelsStatus !== "ready"} onValueChange={setModelKey}>
					<SelectTrigger id="codex-existing-model" aria-describedby="codex-model-help"><SelectValue placeholder={t("chooseModel")} /></SelectTrigger>
					<SelectContent>
						{modelKey && !selected && <SelectItem value={modelKey} disabled>{t("modelMissing")}</SelectItem>}
						{models.map(model => <SelectItem key={model.modelKey} value={model.modelKey} disabled={!!model.unavailable}>
							{model.label}{model.unavailable ? ` — ${t("modelUnavailable")}` : ""}
						</SelectItem>)}
					</SelectContent>
				</Select>
				{modelsStatus === "loading" && <p role="status">{t("modelLoading")}</p>}
				{modelsStatus === "failed" && <p role="alert">{t("modelLoadFailed")}</p>}
				{modelsStatus === "ready" && models.length === 0 && <p role="status">{t("modelEmpty")}</p>}
				{modelKey && modelsStatus === "ready" && (!selected || selected.unavailable) && <p role="alert">{t("modelMissing")}</p>}
				{selected?.baseUrl && <p className="break-all text-muted-foreground">{selected.baseUrl}</p>}
				<p id="codex-model-help" className="text-muted-foreground">{t("reuseHelp")} {t("responsesOnly")}</p>
				{reloadModels && <Button type="button" variant="outline" onClick={reloadModels} disabled={modelsStatus === "loading"}>{t("refreshModels")}</Button>}
			</>}
			<p className="text-muted-foreground">{t("profileWarning")}</p>
			{(["executable", "expectedVersion", "codexHome", "cwd", ...(source === "codex" ? ["model" as const] : [])] as const).map(field => <div key={field}>
			...
			<Select value={value.sandbox} disabled={disabled} onValueChange={next => setValue(previous => ({ ...previous, sandbox: next === "workspace-write" ? "workspace-write" : "read-only" }))}>
				<SelectTrigger id="codex-sandbox"><SelectValue /></SelectTrigger>
				<SelectContent>
			...
			<Button type="submit" variant="outline" disabled={!canSave}>{t("save")}</Button>
		</fieldset>
	</form>;
}
