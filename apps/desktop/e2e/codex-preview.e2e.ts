...
installedCodex("installed conversation runtime switch", () => {
	it("switches the new-conversation backend in place without exposing executable setup fields", async () => {
		...
		const codex = await $("button=Codex"); const native = await $("button=Native");
		...
		const installed = await browser.electron.execute(electron => ({ packaged: electron.app.isPackaged,
			home: process.env.VETTA_HOME, testMode: process.env.VETTA_E2E }));
		expect(installed.packaged).toBe(true); expect(installed.testMode).toBe("1"); expect(installed.home).toBeTruthy();
	});
});
