...
- 自动生成的会话标题不再缺字：提示词要求模型写 10–20 个汉字、3–6 个词，而 `sanitizeAutoTitle` 对含 CJK 的标题一律截到 14 字，合规标题会从第 15 个字起被硬截断。改为提示词与截断共用同一份上限（`TITLE_MAX_CHARS_CJK`），并修正纯拉丁标题的词边界收尾——截断点恰好落在词尾时保留完整单词，不再平白回退一个词。
