...
if (required && (!executable || !entry)) throw new Error("Real Codex is required but its executable/entry was not provided");
...
describe.skipIf(!executable || !entry)("pinned real Codex with an isolated local model", () => {
	it("runs a real tool, restores authoritative history, follows up and confirms cancellation", async () => {
		assert.equal(process.platform, "linux", "Run this gate in the Linux network namespace from the validation workflow");
		assert.ok(Object.values(networkInterfaces()).flat().every((network) => !network || network.internal),
		...
		try {
			...
			bridge = await startCodexProviderBridge({
				resolve: async () => ({ identity: "real-binary-contract", revision: "synthetic-key", model: FIXTURE_MODEL,
					baseUrl, headers: { authorization: "Bearer synthetic-contract-key" } }),
				fetch,
			});
			const backend = new CodexRuntimeHostBackend({ catalogRoot: join(root, "catalog"),
				profile: { id: "contract", executable, executableArgs: [entry], expectedVersion: VERSION,
					codexHome, model: FIXTURE_MODEL, gateway: bridge.provider, providerIdentity: bridge.identity,
					requestTimeoutMs: 15000, interruptTimeoutMs: 10000, turnTimeoutMs: 45000 },
			...
			const first = await host.prompt(created.sessionId, { text: "Run the requested read-only contract command, then finish." });
			assert.equal(first.status, "completed");
			assert.equal(model.requests.length, 2, "The first turn must execute the requested tool and return its output to the model");
			...
			assert.equal((await host.prompt(restored.sessionId, { text: "Answer once without running another tool." })).status, "completed");
