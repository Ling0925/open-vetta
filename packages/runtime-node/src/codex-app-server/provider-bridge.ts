...
const fail = (code: string): never => {
	throw new CodexGatewayError(code);
};
...
export async function startCodexProviderBridge(
	source: CodexGatewaySource,
	signal?: AbortSignal,
): Promise<CodexProviderBridge> {
	const initial = structuredClone(await abortableRead(() => source.resolve(), signal ? [signal] : []));
	...
	const current = async (signal?: AbortSignal) => {
		...
		if (closed || invalid || initial.identity !== target.identity || initial.revision !== target.revision) {
			invalidate();
			return fail("MODEL_CONFIGURATION_CHANGED");
		}
		return target;
	};
	let expectedHost = "";
	...
	async function forward(request: IncomingMessage, response: ServerResponse): Promise<void> {
		...
		try {
			...
			try {
				...
				return fail("GATEWAY_BODY_INVALID");
			}
	...
	if (!address || typeof address === "string") {
		unsubscribe?.();
		server.close();
		return fail("GATEWAY_START_FAILED");
	}
	expectedHost = `127.0.0.1:${address.port}`;
	return {
		identity: initial.identity,
		provider: {
			id: `vetta_${randomBytes(12).toString("hex")}`,
			baseUrl: `http://${expectedHost}/v1`,
			bearerToken: token,
			model: initial.model,
			...(initial.reasoning ? { reasoning: initial.reasoning } : {}),
		},
...
export function codexGatewayThreadConfig(provider: CodexGatewayProvider) {
	...
	if (
		...
		url.search ||
		url.hash ||
		!/^vetta_[a-f0-9]{24}$/.test(provider.id) ||
		!/^[a-f0-9]{64}$/.test(provider.bearerToken)
	)
		fail("GATEWAY_BINDING_INVALID");
	return {
