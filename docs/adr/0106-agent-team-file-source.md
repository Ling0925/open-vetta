...
2. 使用现有 `VETTA_HOME` 作为根目录：生产默认 `~/.vetta`，开发/验证当时通过 `VETTA_HOME=~/.vetta-dev` 隔离（后续默认启动策略见 ADR-0131）。领域 ID 一律使用普通 UUID；应用附带数据与用户新建数据使用相同格式，不使用 `builtin:agent:*`、`builtin:team:*` 或其它身份前缀。
